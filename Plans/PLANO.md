...
- **Catálogo de cards** (`catalog.ts`): cada card é um componente "burro" registrado num catálogo (`{ label, allowedSizes, component }`), recebendo só dados/callbacks via props — nunca busca os próprios dados. Isso é o que permite o usuário adicionar/remover cards pelo catálogo sem tocar em código.
...
Cards candidatos para o catálogo inicial do Stockly:
- Total em estoque (quantidade e/ou valor).
- Vendas do dia / mês.
- **Valores recebidos do dia / mês** (decidido — substitui "Ticket médio" no dashboard): difere de "Vendas do dia/mês" porque uma venda em **Crediário** conta como venda no momento do registro, mas o dinheiro só entra quando a dívida é quitada (ver "Crediário e Devedores"). Cálculo: pagamentos em Dinheiro/Cartão/PIX registrados no período + quitações de Crediário (parciais/totais) recebidas no período — **não** inclui o valor de vendas em Crediário ainda em aberto.
- Itens com estoque baixo.
- Categoria(s) mais vendida(s).
- Gráfico de vendas ao longo do tempo (Chart.js).
- **Devedores com lembrete próximo/vencido** (Crediário) — ver seção "Crediário e Devedores".
- (fase 2) Card de "Sugestões" em destaque, puxando da tela de Sugestões — ver "Ideias extras".
...
4. **Implementar em fatias verticais por feature** (decidido) — schema + comando Rust + tela, um recurso de cada vez, em vez de fechar todo o backend e só depois todo o frontend (ou vice-versa): os mockups HTML já definem a UI em detalhe e o `PLANO.md` já define as regras de negócio em detalhe, então separar as fases só adicionaria risco de integração no fim. Ordem sugerida, seguindo a dependência natural entre os recursos:
   ...
   2. ~~Estoque (cadastro de item, categorias) — Venda precisa de item cadastrado pra existir.~~ — **concluído**, exceto Importar/Exportar CSV (fica no item 6, "funcionalidades de apoio") — ver `docs/commands.md`.
   ...
   5. **Histórico de vendas (listagem + busca) e Cancelamento/Estorno** — fecha o fluxo completo de venda começado no item 3 (sem essa tela não dá pra cancelar/estornar nada hoje). Não tem tela própria nos mockups (só é mencionada como pré-requisito nas seções "Histórico de vendas com busca" e "Cancelamento / Estorno de venda", acima em "Vendas") — não precisa criar mockup pra ela, seguir direto pelas regras já descritas ali. Cobre: listar/buscar vendas (por data/período, número do recibo, cliente quando Crediário, usuário que registrou) e, a partir de uma venda localizada, cancelar/estornar (senha de admin, devolve itens ao estoque, nunca apaga o histórico — só marca como `cancelled`).
   6. Dashboard e Relatórios — depende de ter dado de venda acumulado pra mostrar algo.
   7. Backup, autoupdate, CSV — funcionalidades de apoio, não bloqueiam nada acima.
