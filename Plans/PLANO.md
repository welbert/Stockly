...
**Status (atualizado após a implementação começar)**: menu lateral expansível implementado (`AppShell`, ver `docs/frontend.md`), com todos os 13 relatórios do catálogo prontos (6 de Vendas — incluindo Descontos concedidos, adicionado depois do lote original —, 3 de Estoque, 3 de Crediário/Devedores, 1 de Auditoria — marcados "Implementado" acima). Ticket médio por período, Vendas canceladas/estornadas, Margem/lucro, Estoque valorizado e Previsão de ruptura continuam fora do menu por enquanto — nenhum item além do já marcado "Implementado" está priorizado/comprometido.
...
4. **Implementar em fatias verticais por feature** (decidido) — schema + comando Rust + tela, um recurso de cada vez, em vez de fechar todo o backend e só depois todo o frontend (ou vice-versa): os mockups HTML já definem a UI em detalhe e o `PLANO.md` já define as regras de negócio em detalhe, então separar as fases só adicionaria risco de integração no fim. Ordem sugerida, seguindo a dependência natural entre os recursos:
   ...
   8. Backup, autoupdate, CSV — funcionalidades de apoio, não bloqueiam nada acima. ~~CSV~~ e ~~Backup~~ **concluídos** (ver `docs/commands.md`, `docs/frontend.md`); falta só **autoupdate**.
