...
| `apply_items_csv_import` | `(decision: ItemsCsvImportDecision) -> ItemsCsvImportResult` | admin-only, the only command that writes. `decision.creates`/`updates` carry the exact field values to apply (trusted the same way `update_item` trusts an admin's form, since the review screen already showed the diff) — `updates` covers both code-matched rows and "new" rows the admin manually remapped to an existing item (`keepExistingName: true` for the latter, so a CSV typo can't rename the real item, per `Plans/PLANO.md`'s import section). `missingActions` applies `"zero"`/`"deactivate"` to items left out of the CSV (`"keep"` is a no-op). One transaction for the whole import. Quantity deltas are logged as `stock_movements` type `csv_import`; price changes go to `item_price_history`, same as `create_item`/`update_item`. A category named in a row that doesn't exist yet is created on the fly (find-or-create by exact name — safe since this command is already admin-only) |
